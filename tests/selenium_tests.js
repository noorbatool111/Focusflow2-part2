...
describe('FocusFlow Automated Test Suite', function () {
    ...
    async function dismissAlertIfOpen() {
        try {
            let alert = await driver.switchTo().alert();
            await alert.accept();
        } catch (e) { /* no alert open */ }
    }
    ...
    it('1. Should successfully sign up a new user', async function () {
        ...
        // Wait for and handle the success alert (server round-trip needs time)
    ...
    it('2. Should show error for duplicate email signup', async function () {
        ...
        const alertText = await handleAlertIfPresent(10000);
        // Alert should indicate the email already exists
        if (alertText) expect(alertText.toLowerCase()).to.satisfy(t => t.includes('exists') || t.includes('already') || t.includes('wrong'));
    });
    ...
    it('5. Should show error for incorrect password', async function () {
        ...
        // App may return 'invalid' or 'not found' depending on implementation
        if (alertText) expect(alertText.toLowerCase()).to.satisfy(t => t.includes('invalid') || t.includes('not found') || t.includes('incorrect') || t.includes('wrong'));
    });
    ...
    it('9. Should successfully add a new note', async function () {
        ...
        await driver.wait(until.elementLocated(By.id('note-textarea')), 10000);
    ...
    it('12. Should successfully add a new task', async function () {
        ...
        await driver.wait(until.elementLocated(By.id('task-input')), 10000);
    ...
    it('14. Should successfully submit feedback', async function () {
        ...
        await driver.wait(until.elementLocated(By.id('feedback-name')), 10000);
        ...
        await driver.wait(until.elementLocated(By.id('feedback-success')), 10000);
